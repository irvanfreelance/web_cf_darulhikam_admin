...
export default function Page() {
  const { data, error, isLoading } = useSWR('/api/web-banks', fetcher);
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title="Manajemen Rekening Resmi" 
        description="Kelola data Rekening Resmi untuk website utama"
      >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden text-left p-6">
        {isLoading ? (
          ...
          <div className="text-center py-12 text-slate-400 font-medium">Belum ada data Rekening Resmi.</div>
        )}
      </div>
    </div>
  );
