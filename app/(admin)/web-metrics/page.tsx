...
export default function MetricsPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(err.error || 'Gagal menyimpan data metric');
      }
    } catch (e) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/web-metrics?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
        mutate();
      } else {
        const err = await res.json();
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title="Impact Metrics (Capaian Dampak)" 
        description="Kelola angka pencapaian dampak (penyalaan counter angka di beranda website)"
      >
        <Button onClick={handleOpenCreate} className="shrink-0">
          <Plus size={18} strokeWidth={3} className="mr-2" /> Tambah Metric
        </Button>
      </PageHeader>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden p-6 space-y-6">
        <div className="flex justify-between items-center">
          <div className="relative max-w-sm w-full">
            ...
            <Input 
              placeholder="Cari label atau metric key..." 
            ...
            <table className="w-full text-left border-separate border-spacing-0">
              <thead>
                <tr className="bg-slate-50 text-slate-500 text-[11px] font-bold uppercase tracking-wider">
                  <th className="px-6 py-4 rounded-tl-xl">Metric Key</th>
                  <th className="px-6 py-4">Label Display</th>
                  <th className="px-6 py-4">Nilai Angka</th>
      ...
      <Modal
        ...
        title={editingItem ? 'Edit Impact Metric' : 'Tambah Impact Metric Baru'}
        description="Isi informasi metric capaian dampak"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Metric Key (Unik) *</label>
              <Input 
                required
                placeholder="total_funds_distributed_m"
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Label Tampilan *</label>
              <Input 
                required
                placeholder="Dana Tersalurkan (Rp)"
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Nilai Angka (Value) *</label>
              <Input 
                type="number"
                step="any"
                required
            ...
            <div>
              <label className="block text-xs font-bold text-slate-700 mb-1">Suffix (Akhiran, misal: M+, rb+, +)</label>
              <Input 
                placeholder="M+"
