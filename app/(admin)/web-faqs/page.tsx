...
import { HelpCircle, Plus, Search, Edit2, Trash2, Save, Loader2 } from 'lucide-react';
...
export default function FaqsPage() {
  ...
  const { data: faqs, error, mutate, isLoading } = useSWR(`/api/web-faqs?search=${search}`, fetcher);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(err.error || 'Gagal menyimpan FAQ');
      }
    } catch (e) {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm('Hapus pertanyaaan FAQ ini?')) return;
    try {
      const res = await fetch(`/api/web-faqs?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
        mutate();
      } else {
        const err = await res.json();
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        ...
        description="Kelola daftar pertanyaan yang sering diajukan (FAQ) di website utama"
      >
        <Button onClick={handleOpenCreate} className="shrink-0">
          <Plus size={18} strokeWidth={3} className="mr-2" /> Tambah FAQ
        </Button>
      </PageHeader>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden p-6 space-y-6">
        ...
        {isLoading ? (
            ...
            <table className="w-full text-left border-separate border-spacing-0">
              <thead>
                <tr className="bg-slate-50 text-slate-500 text-[11px] font-bold uppercase tracking-wider">
                  <th className="px-6 py-4 rounded-tl-xl max-w-md">Pertanyaan (Question)</th>
                  <th className="px-6 py-4">Kategori</th>
                  <th className="px-6 py-4 max-w-md">Jawaban (Answer)</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {faqs.map((item: any) => (
                  <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-6 py-4 text-sm font-semibold text-slate-800 max-w-md">
                      {item.question}
                    </td>
                    <td className="px-6 py-4">
                      <span className="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide bg-indigo-50 text-indigo-600 border border-indigo-100">
                        {item.category}
                      </span>
                    </td>
          ...
          <div className="text-center py-12 text-slate-400 font-medium">Belum ada data FAQ.</div>
        )}
      </div>
      ...
      <Modal
        ...
        description="Isi detail pertanyaan dan jawaban FAQ"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input 
              required
              placeholder="Contoh: Apakah LAZ Darul Hikam sudah berizin resmi?"
          ...
          <div>
            ...
            >
              ...
              <option value="donation">Donasi & Transfer</option>
              <option value="zakat">Zakat & Nisab</option>
              <option value="program">Program & Penyaluran</option>
              <option value="partnership">Kemitraan</option>
              <option value="technical">Teknis & Akun</option>
            </select>
          </div>
