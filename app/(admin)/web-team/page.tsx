...
import { Users, Plus, Search, Edit2, Trash2, Save, Loader2, User } from 'lucide-react';
...
export default function TeamPage() {
  ...
  const { data: team, error, mutate, isLoading } = useSWR(`/api/web-team?search=${search}`, fetcher);
  ...
  const handleOpenEdit = (item: any) => {
    ...
    setFormData({
      ...
      bio: item.bio || '',
      accent_color: item.accent_color || '#3268C3',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(err.error || 'Gagal menyimpan data pengurus');
      }
    } catch (e) {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const res = await fetch(`/api/web-team?id=${id}`, { method: 'DELETE' });
      if (res.ok) {
        mutate();
      } else {
        const err = await res.json();
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title="Tim & Pengurus Organisasi" 
        description="Kelola susunan direksi, pembina, pengawas, dan manajer eksekutif LAZ"
      >
        <Button onClick={handleOpenCreate} className="shrink-0">
          <Plus size={18} strokeWidth={3} className="mr-2" /> Tambah Pengurus
        </Button>
      </PageHeader>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden p-6 space-y-6">
        ...
        {isLoading ? (
            ...
            <table className="w-full text-left border-separate border-spacing-0">
              <thead>
                <tr className="bg-slate-50 text-slate-500 text-[11px] font-bold uppercase tracking-wider">
                  <th className="px-6 py-4 rounded-tl-xl">Nama & Foto</th>
                      ...
                      <div className="flex items-center gap-3">
                        {item.avatar_url ? (
                          ...
                          <div 
                            ...
                            style={{ backgroundColor: item.accent_color || '#3268C3' }}
                          >
          ...
          <div className="text-center py-12 text-slate-400 font-medium">Belum ada pengurus.</div>
        )}
      </div>
      ...
      <Modal
        ...
        description="Isi profil lengkap pengurus"
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <Input 
              required
              placeholder="Contoh: Ust. Ahmad Habib, Lc."
              value={formData.name}
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input 
                placeholder="Contoh: Eksekutif / Program"
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">Biografi Singkat (Opsional)</label>
            <textarea 
              rows={3}
              ...
              placeholder="Ringkasan pengalaman atau latar belakang..."
              value={formData.bio}
