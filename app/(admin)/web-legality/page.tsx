...
export default function Page() {
  const { data, error, isLoading } = useSWR('/api/web-legality', fetcher);
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title="Manajemen Legalitas" 
        description="Kelola data Legalitas untuk website utama"
      >
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden text-left p-6">
        {isLoading ? (
          ...
          <div className="text-center py-12 text-slate-400 font-medium">Belum ada data Legalitas.</div>
        )}
      </div>
    </div>
  );
